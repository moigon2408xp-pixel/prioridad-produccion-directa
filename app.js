...
function formOrder() {
  ...
  $("#order-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; const button = form.querySelector("button[type=submit]"); button.disabled = true; button.textContent = "Registrando…"; try { const values = Object.fromEntries(new FormData(form)); if (!String(values.cliente || "").trim() || !String(values.tipo || "").trim()) throw new Error("Completa cliente y tipo de trabajo."); if (!String(values.fechaEntrega || "").trim() || !String(values.horaEntrega || "").trim()) throw new Error("Completa fecha y hora de entrega."); await api("profile_create_order", { form: values }); closeModal(); await refresh(false); showToast("Pedido registrado."); } catch (error) { button.disabled = false; button.textContent = "Registrar"; window.alert(`No se pudo registrar el pedido.\\n\\nDetalle: ${error.message}`); } });
}
