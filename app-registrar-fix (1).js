...
async function imageAsJpegData(file) {
  ...
  try {
    const source = await new Promise((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("El teléfono no pudo leer la fotografía seleccionada."));
      image.src = objectUrl;
    });
    const maxSide = 1100;
    ...
    if (!context) throw new Error("No se pudo preparar la fotografía en este teléfono.");
    context.drawImage(source, 0, 0, canvas.width, canvas.height);
    let quality = 0.68;
    let data = canvas.toDataURL("image/jpeg", quality);
    while (data.length > 2200000 && quality > 0.42) {
      quality -= 0.08;
      data = canvas.toDataURL("image/jpeg", quality);
    }
    if (data.length > 2200000) {
      throw new Error("La fotografía sigue siendo demasiado grande. Toma una foto más cercana o con menor resolución.");
    }
    return data;
  } finally {
